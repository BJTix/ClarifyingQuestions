...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        conn = get_conn()
        cursor = conn.cursor()
        sql = f"UPDATE StudyResults SET OverallPreference2 = {Preference}, UsefulnessPreference2 = {Usefulness} \
, DocBaselineFeedback2 = '{BaselineFeedback}', DocQAFeedback2 = '{QAFeedback}' WHERE SessionID = {SessionID}"
        cursor.execute(sql)
        conn.commit()
